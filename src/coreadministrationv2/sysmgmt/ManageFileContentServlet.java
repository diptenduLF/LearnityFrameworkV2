...
@WebServlet("/coreadministrationv2.sysmgmt.ManageFileContent") 
public class ManageFileContentServlet extends LfServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		if (_SAVE_OPERATION.equals(operation)) {
			statusMessage = saveFileContent(request, response, this.resourceIdInput, interfaceId,templateId,type,fileName);
		}
	...
	private String saveFileContent(HttpServletRequest request, 
...
//		String fileName = null;
		...
		try {
			FileUtils.writeStringToFile(new File(filePath.concat(fileName)), contentData, "UTF-8");
...
//				Pair<String,String> returnPair=ApplicationTemplateManagement.uploadTemplateXML(request, resourceId, templateId, filePath, fileName, size.toString(), defaultValue);
				statusMessage=ResourceApplicationTemplate.uploadTemplateXml(request, templateId, filePath, "application-template.xml", fileName, defaultValue);
//				statusMessage=returnPair.getFirst();
//				this.resourceIdInput=returnPair.getSecond();

			} else if (_TEMPLATE_RESOURCE_TYPE.equalsIgnoreCase(type)) {
				statusMessage=ResourceApplicationTemplate.uploadTemplateResource(request, templateId, filePath, fileName);
...
//					 String attachmentname1= removeChar(attachmentname);
					...
					String resourcePath = contextPath+File.separator+attachmentname+File.separator+fileName;
		...
		if (GenericUtil.hasString(fileString)) {
			String htmlContent = org.apache.commons.text.StringEscapeUtils.escapeHtml4(fileString);
			form.addElement("<textarea rows=\"150\" cols=\"93\" name=\"file_content\">" + htmlContent + "</textarea>");
		}

		if (_THEME_TYPE.equalsIgnoreCase(type) || _TEMPLATE_TYPE.equalsIgnoreCase(type) ||  _TEMPLATE_RESOURCE_TYPE.equalsIgnoreCase(type)) {
		...
		form.addElement("<input type=\"hidden\" name=\"template_id\" value=\"" + templateId + "\">");
		form.addElement("<input type=\"hidden\" name=\"delivery_mode\" value=\"" + deliveryMode + "\">");
		form.addElement("<input type=\"hidden\" name=\"file_name\" value=\"" + fileName + "\">");
		
		String contextPath = request.getContextPath();
		createManageFileContentOperations(html, form, formName, templateId, deliveryMode, type, contextPath, fileName);
...
*/
	private void createManageFileContentOperations(Html html, Form form, String formName, String templateId, 
			String deliveryMode, String type, String contextPath, String fileName) {

		
		String javaScript = null;

		if ((deliveryMode != null) && (deliveryMode.equals("URIPath")))
			javaScript = "function saveFileContent() {alert('For resources with Delivery Mode URIPath saving not supported');}";
		else {
			javaScript = "\n	function saveFileContent() {" + "\n			document." + formName + ".method=\"post\";" + "\n			document." + formName + ".target=\"_self\";"
				+ "\n			document." + formName + ".action = \"coreadministrationv2.sysmgmt.ManageFileContent?operation=" + _SAVE_OPERATION + "\";" + "\n			document." + formName
				+ ".submit();" + "\n	}" + "\n";
			
/*
			String url = contextPath +
    		"/coreadministrationv2.sysmgmt.ManageFileContent?template_id="+templateId+"&type1="+type+ "&operation=" 
					+ _SAVE_OPERATION + "&file_name=" + fileName + "&delivery_mode=" + deliveryMode
					+ "&file_content='";
			javaScript = "function saveFileContent() {var content = encodeURIComponent(document.viewResourceForm.file_content.value); window.location.href = '" + url + " +content+\"'\"}";
*/		}
