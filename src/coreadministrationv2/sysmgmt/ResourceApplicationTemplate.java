 ...
 */
@WebServlet("/coreadministrationv2.sysmgmt.ResourceApplicationTemplate") 
public class ResourceApplicationTemplate extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String type1 = null;
		String __rows = null;
		String __action = null;
		String __field_info = null;
		String searchstring = null;
		String __gridpro1 = null;
		String __gridpro_search = null;
		String __gridpro2 = null;
        ...
        statusMessage = request.getParameter("statusMessage");
      	...
      	//if (strAdministratorPreviledge != null) {
	        	if (strPrmAddModDel!=null) {
			        ...
			        String htmlString = "<!DOCTYPE html> <html><head>"
			        		...
			        		"template_id="+template_id+"&statusMessage="+statusMessage+
			        		"' </script>" + 
			        		"</head>" + 
			        		"<body></body></html>";

					out.print(htmlString);
					return;					//avoid form submission by JSPGrid
	        	}
...
/*	         
	 				if (template_id == null) {
						...
						multipartrequest = 
								new MultipartRequest(request, resourcePath, 50 * 1024 * 1024);
						for (Enumeration enumeration = multipartrequest.getParameterNames(); 
								enumeration.hasMoreElements();) {
							String parameterName = (String) enumeration.nextElement();
							String paratemeterValue = multipartrequest.getParameter(parameterName);
						}
						template_id = multipartrequest.getParameter("template_id");
						type1 = multipartrequest.getParameter("type1");
						__rows = multipartrequest.getParameter("__rows");
						__action = multipartrequest.getParameter("__action");
						__field_info = multipartrequest.getParameter("__field_info");
						searchstring = multipartrequest.getParameter("searchstring");
						__gridpro1 = multipartrequest.getParameter("__gridpro1");
						__gridpro_search = multipartrequest.getParameter("__gridpro_search");
						__gridpro2 = multipartrequest.getParameter("__gridpro2");
						selectedfilename = multipartrequest.getParameter("selectedfilename");
						selecteddeliverymode = multipartrequest.getParameter("selecteddeliverymode");
						selectedassetpath = multipartrequest.getParameter("selectedassetpath");
						filename = multipartrequest.getParameterValues("filename");
						deliverymode = multipartrequest.getParameterValues("deliverymode");
						assetpath = multipartrequest.getParameterValues("filename");
...
*/	 
	       ...
	       Html html = new Html()
					.addElement(new Head()
					.addElement(new Title("Resource Application Template Management"))
					.addElement(new Link()
...
//			Form form = new Form().setName("frm").setMethod("post").setEncType("multipart/form-data");
			...
			if(invalid != null && invalid.equals("1"))
			{
				...
				Input styleid= new Input().setType("file").setName("uploadfilename").setClassId("PPRLabelText");
				...
				if	(type1.equals("Resources"))
			      {
					sql="select application_template_id as `select` ,file_name as `File Name` , delivery_mode as `Delivery Mode` , asset_type as `Asset Type` , sequence_no as `Seq No` ,asset_path as `Asset Path`, CONCAT(ROUND(length(template_asset_file)/1024,2),' KB') as `Asset Size`  from application_template_asset where application_template_id='"+template_id+"'";
				}
				...
				try {
					...
					grid1.setASCImageSource("/images/asc.gif");
					grid1.setDESCImageSource("/images/desc.gif");
...
public String add(HttpServletRequest request, String strCreatedBy, PrintWriter out1, HttpServletResponse response, String type1 )
						throws IOException, ServletException {
	String template_id=request.getParameter("template_id");
	String statusMessage=uploadTemplate(request, response, strCreatedBy, out1, template_id, type1);
	return statusMessage;
}

public static String uploadTemplate(HttpServletRequest request, HttpServletResponse response, String strCreatedBy, PrintWriter out1, String template_id,  String type1) throws IOException, ServletException {
	...
	String tempFileName="application-template.xml";
...
//	attachmentname = path;
	try {
			MultipartRequest multipartrequest = new MultipartRequest(request, path, 50 * 1024 * 1024);
	...
	if (type1.equals("Resources"))
		statusMessage = uploadTemplateResource(request, template_id, path, fileName);
	else if(type1.equals("Templatexml"))
		statusMessage =uploadTemplateXml(request,template_id,path,tempFileName,s7,default_value);
...
static String uploadTemplateResource(HttpServletRequest request, String template_id, String path, String fileName) {
					
	Pair<Boolean, String> returnStatus=new Pair<>();
	String statusMessage = "";
	try {
                     
           returnStatus = DataBaseLayer.updateTemplateResource(template_id,path,fileName);
...
public static synchronized String uploadTemplateXml(HttpServletRequest request,String template_id,String path,String tempFileName,String fileName, String default_value)
{
	 String  inFileName=path+fileName; 
	 String pathname=path+tempFileName;
	 String statusMessage="";
...
//				 isSuccess=coreadministrationv2.dbconnection.DataBaseLayer.TemplateInsert(template_title,attachmentname,s7,strSize,uiFramework,blockUITimeout);
				 isSuccess=coreadministrationv2.dbconnection.DataBaseLayer.TemplateUpdate(template_id,template_title,path,fileName,uiFramework,blockUITimeout);
				 if(isSuccess){
...
//								 coreadministrationv2.dbconnection.DataBaseLayer.InsertApplicationDefaultValue(current_template_id,"","Configuration",configuration_attribute_name,configuration_attribute_value);
								 coreadministrationv2.dbconnection.DataBaseLayer.UpdateApplicationDefaultValue(template_id,"","Configuration",configuration_attribute_name,configuration_attribute_value);
							 }
						 }
...
//										 coreadministrationv2.dbconnection.DataBaseLayer.InsertApplicationDefaultValue(current_template_id,class_name,section_name,attribute_name,attribute_value);
										 coreadministrationv2.dbconnection.DataBaseLayer.UpdateApplicationDefaultValue(current_template_id,class_name,section_name,attribute_name,attribute_value);
									 }
								 }
						 ...
						 for(int e=0; e<framework_asset_delivery_list.getLength() ; e++)
						 {
							 ...
							 for(int f=0; f<asset_list.getLength() ;f++)
							 {
								 ...
								 String asset_value= asset_list_element.getAttribute("asset_value");
				                  ...
				                  coreadministrationv2.dbconnection.DataBaseLayer.UpdateTemplateAsset(template_id,type,deliverymode,pagelocation,deliverysequence,filename,assetpath,pathname);
							 }
						 }
					 }else{
