...
@WebServlet("/interfaceenginev2.DBGridQueryEditorServlet") 
...
// 					System.out.println("===============sql_query=========="+sql_query);
	...
	if(oper.equals("add"))
	{
		...
		for(int i=0;i<no_of_custom_validation_query;i++)
		{
			...
			//System.out.println("=======================custom_validation_class===ADD="+custom_validation_class);
			Pair<Boolean, String> custom_flag = null;
			boolean validation=false;
			String message="";
			...
			if(custom_flag!=null){
				validation = custom_flag.getFirst();
				message = custom_flag.getSecond();
				//System.out.println("=======validation===="+validation+"========message==ADD======"+message);
				if(!validation)
				{
		...
		//System.out.println("Custom validation flag==============="+flag);
...
// 						System.out.println("??????????????????????REPLACE ELSE??????????????????"+stringtoreplace);
 					...
 					System.out.println("===Grid edit query====="+sql_query);

					returnStatus = NewDataBaseLayer.ExecuteSqlQuery(sql_query);
				}
				...
				if(returnStatus==true)		
					...
					out.println(GenericUtil.generateJsonString(GridModificationStatus.ERROR_STATUS, "Record Insertion Failed"));
...
// 					System.out.println("===============custom_action_flag in delete========"+custom_action_flag);
			...
			else
			{
				...
				for(int j=0;j<no_of_query;j++)
				{
					...
					parameter_title=NewDataBaseLayer.getModifyParameter(interface_id,part_id,j+1);
					parameter_title_vector = StringtoVector(parameter_title);
					//System.out.println("parameter_title_vector========="+parameter_title_vector);
					String stringreplacewith ="";
...
//	mysession.setAttribute("namepairvalue", namepairvalueforexcel);
